...
# The Data Preparation process consists of two primary activities: Loading Data and Data Wrangling. We will use the already downloaded data set from Kaggle. The link for downloading of data is mentioned above. The data set with file name heart.csv has been uploaded in the GitHub so that the data is universally availble at the time of running the code by all the stake holders. We will use the read.csv function to read the raw data into RStudio using the GitHub link. Following this, we will perform the  Data Wrangling, where we perform data cleaning, transformation, and structuring tasks, ensuring that the data is in an optimal and reliable state for further analysis and model development.

# i. Load Data

heart <- read.csv("https://raw.githubusercontent.com/georgebhilai/Heart_Risk_Prediction_Model/main/heart.csv")
heart %>% head()
...
# While going through the data we find many variables that need to be converted into factor data types. These variables include sex, cp, fbs, restecg, exang, slope, ca, thal and target. Being categorical variables if the data types are made factor type we will be able to handle these categorical variables in our analysis and modeling, and also help us to better comprehend through visualizations.
...
# The Exploratory Data Analysis (EDA) section of our project gives insights from the dataset.
...
# The distribution of the target class in dataset shows that it is balanced, with approximately 48.68% of instances falling into category 0 and around 51.32% into category 1, there is no significant imbalance between the two classes. A balanced distribution of the target class is a favorable sign for our analysis and modeling processes. It ensures that both outcomes are adequately represented in dataset.
...
# We are keeping the ratio as 75 : 25 since we need the training set to be large enough to train the model. The training is where most data is needed so that the model can reach its full potential and the test set is a much smaller amount of data to help us choose which of the models will perform best on unseen data. 
...
# The model results indicate the relationship between the target variable (presence or absence of heart disease) and several predictor variables. The coefficients associated with each predictor variable represent their impact on the target variable.
